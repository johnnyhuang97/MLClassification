...
class RegressionModel(Model):
    ...
    def run(self, x, y=None):
        ...
        xw1_plus_b = nn.MatrixVectorAdd(graph, xw1, self.b1)

        xr = nn.ReLU(graph, xw1_plus_b)

        xw2 = nn.MatrixMultiply(graph, xr, self.w2)
        xw2_plus_b = nn.MatrixVectorAdd(graph, xw2, self.b2)
        ...
        if y is not None:
            ...
            loss = nn.SquareLoss(graph, xw2_plus_b, input_y)
            return graph
        else:
            # At test time, the correct output is unknown.
            # You should instead return your model's prediction as a numpy array
            "*** YOUR CODE HERE ***"
            return graph.get_output(xw2_plus_b)
...
class OddRegressionModel(Model):
    ...
    def run(self, x, y=None):
        ...
        neg_ones = -1 * np.ones((1,))

        input_negx = nn.Input(graph, neg_ones * x)

        new_x = applyfx(input_negx)

        input_neg = nn.Input(graph, np.array((neg_ones,)))

        neg = nn.MatrixMultiply(graph, new_x, input_neg)

        added = nn.Add(graph, pos, neg)
        ...
        if y is not None:
            ...
            "*** YOUR CODE HERE ***"
            input_y = nn.Input(graph, y)
            loss = nn.SquareLoss(graph, added, input_y)
            return graph
        else:
            # At test time, the correct output is unknown.
            # You should instead return your model's prediction as a numpy array
            "*** YOUR CODE HERE ***"
            return graph.get_output((added))
...
class DigitClassificationModel(Model):
    ...
    def run(self, x, y=None):
        ...
        def applyfx(input_x):
            ...
            xr1 = nn.ReLU(graph, xw1_plus_b)

            xw2 = nn.MatrixMultiply(graph, xr1, self.w2)
            xw2_plus_b = nn.MatrixVectorAdd(graph, xw2, self.b2)

            xr2 = nn.ReLU(graph, xw2_plus_b)

            xw3 = nn.MatrixMultiply(graph, xr2, self.w3)
            xw3_plus_b = nn.MatrixVectorAdd(graph, xw3, self.b3)
        ...
        if y is not None:
            "*** YOUR CODE HERE ***"
            input_y = nn.Input(graph, y)
            loss = nn.SoftmaxLoss(graph, pos, input_y)
            return graph
        else:
            "*** YOUR CODE HERE ***"
            return graph.get_output((pos))
...
class DeepQModel(Model):
    ...
    def run(self, states, Q_target=None):
        ...
        if Q_target is not None:
            "*** YOUR CODE HERE ***"
            input_qt = nn.Input(graph, Q_target)
            loss = nn.SquareLoss(graph, pos, input_qt)
            return graph
        else:
            "*** YOUR CODE HERE ***"
            return graph.get_output((pos))
...
class LanguageIDModel(Model):
    ...
    def __init__(self):
        ...
        self.hidden_size = 175
        self.w1 = nn.Variable(47,self.hidden_size)
        self.w2 = nn.Variable(self.hidden_size,self.hidden_size)
        self.w3 = nn.Variable(self.hidden_size,5)
    ...
    def run(self, xs, y=None):
        ...
        vector = np.zeros(self.hidden_size)
        h = nn.Input(graph, np.array((vector,) * batch_size))

        def applyfx(h, c):
            input_c = nn.Input(graph, c)
            xw1 = nn.MatrixMultiply(graph, input_c, self.w1)
            xw1_plus_b = nn.MatrixVectorAdd(graph, xw1, self.b1)

            x_plus_h = nn.Add(graph, xw1_plus_b, h)
            xr1 = nn.ReLU(graph, x_plus_h)
        ...
        pos = nn.MatrixVectorAdd(graph, xw3, self.b3)
