...
class Graph(object):
    ...
    def get_output(self, node):
        ...
        return self.outputs[get_index(node)]
    ...
    def add(self, node):
        ...
        accumulator = np.zeros_like(output)
    ...
    def backprop(self):
        ...
        self.accumulators[num_nodes - 1] = 1.0

        for n, g in zip(reversed(self.nodes), reversed(self.accumulators)):
            backward_pass = n.backward(self.get_inputs(n), g)
            for p, b in zip(n.get_parents(), backward_pass):
                index = get_index(p)
                self.accumulators[index] += b
    ...
    def step(self, step_size):
        ...
        for n in self.get_nodes():
            if n in self.variables:
                index = get_index(n)
                n.data -= step_size * self.accumulators[index]
...
class ReLU(FunctionNode):
    ...
    def backward(inputs, gradient):
        "*** YOUR CODE HERE ***"
        y = np.maximum(inputs[0], 0)
        y[y<=0] = 0
        y[y>0] = 1
        return [np.array(gradient * y)]
...
class SquareLoss(FunctionNode):
    ...
    def backward(inputs, gradient):
        "*** YOUR CODE HERE ***"
        return [gradient*(inputs[0] - inputs[1])/inputs[0].size, gradient*(inputs[1] - inputs[0])/inputs[0].size]
